...
flowchart TD
    ...
    Input --> Norm[RMSNorm]
    Norm --> Router{Router<br/>Linear E×d}
    ...
    Experts --> Output1[Expert outputs<br/>shape: N×k, d]
    Output1 --> Combine
    Combine --> ResidualAdd[+ input residual]
...
```mermaid
flowchart TB
    Token[Token vector<br/>d=7168] --> Router[Router<br/>E×d weight]
    Router --> Scores[Score: 256 experts<br/>after sigmoid]
    Scores --> Bias[+ bias correction]
    Bias --> Reshape[Reshape: 8 groups × 32 experts]
...
```mermaid
flowchart TD
    Router[Router<br/>router_logits N×E] --> Softmax[Softmax → p<br/>routing probs]
    Softmax --> TopK[Top-k indices]
    Softmax --> P[Mean P_i = average prob]
    TopK --> OneHot[one_hot → expert mask]
    OneHot --> F[Fraction f_i = mean mask]

    P --> Dot[Compute f · P sum]
    F --> Dot
    Dot --> AuxLoss["aux = E × sum(f × P)<br/>scalar"]
    AuxLoss --> Scale["× router_aux_loss_coef (0.001)"]
    Scale --> Total[+ to total loss]
